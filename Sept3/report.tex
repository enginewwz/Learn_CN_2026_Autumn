% !TeX program = xelatex
% WSL 推荐编译：latexmk -xelatex -shell-escape -outdir=build report.tex
% minted 需要系统 PATH 中存在 pygmentize
...
\noindent\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}llll@{}}
  姓名：吴峥 & 学号：2024K8009909013 & 班级：2406 & 实验日期：2026 年 9 月 4 日
\end{tabular*}}
\vspace{0.3em}
\hrule
\vspace{1.2em}
...
\begin{table}[H]
  ...
  \small
  \caption{实验涉及的主要协议}
  \begin{tabularx}{\textwidth}{>{\bfseries\centering\arraybackslash}p{1.5cm}L{5.0cm}Y}
    \toprule
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{L{2.6cm}N{2.0cm}Y}
    \toprule
...
\begin{table}[H]
  ...
  \small
  \caption{TCP 三次握手}
  \begin{tabularx}{\textwidth}{N{1.0cm}L{3.0cm}L{2.1cm}L{3.2cm}Y}
    \toprule
...
\begin{table}[H]
  ...
  \small
  \caption{TLS 握手过程}
  \begin{tabularx}{\textwidth}{N{1.4cm}L{4.4cm}Y}
    \toprule
...
\begin{table}[H]
  ...
  \small
  \caption{TCP 连接关闭过程}
  \begin{tabularx}{\textwidth}{N{1.0cm}L{3.2cm}L{2.2cm}Y}
    \toprule
...
\begin{table}[H]
  ...
  \caption{数据清洗结果}
  \label{tab:cleaning}
  \begin{tabularx}{\textwidth}{L{4.0cm}N{2.4cm}Y}
    \toprule
...
\begin{table}[H]
  ...
  \caption{各项指标的中位数}
  \label{tab:medians}
  \begin{tabularx}{\textwidth}{L{3.4cm}N{2.8cm}N{3.0cm}Y}
    \toprule
    \rowcolor{TableHeader} 指标 & 时间中位数 & 膨胀倍数中位数 & 简单理解 \\
    \midrule
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{L{3.4cm}CCCC}
    \toprule
