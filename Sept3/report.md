...
本地实验在 Windows Subsystem for Linux 2（WSL 2）中完成，具体环境如下：
...
| Wireshark | 4.2.2 |
| Python | 3.14.6 |
| 主要 Python 库 | `pycurl 7.47.0`、`geoip2 5.3.0`、`matplotlib 3.11.1` |
| 其他工具与数据 | `wget`、GeoLite2 City IP 地理位置数据库 |
...
在 Mininet 中打开主机 `h1` 的终端：

```text
mininet> xterm h1
```

在 `h1` 中配置 DNS 服务器并启动 Wireshark：
...
停止抓包并保存为 `download-wire-shark.pcapng`。该文件包含 75 个数据包，抓包时长约为 5.14 秒。
...
| DNS | 将容易记忆的域名解析为 IP 地址 | 将 `www.ucas.ac.cn` 解析为 `124.16.77.5` |
| TCP | 在通信双方之间建立可靠的字节流连接 | 与服务器的 443 端口完成握手、传输和关闭连接 |
| HTTP | 规定浏览器或下载工具与 Web 服务器之间的请求和响应格式 | 本次请求使用 HTTPS，因此 HTTP 内容没有以明文显示 |
...
| 阶段 | 数据包编号 | 观察结果 |
| --- | ---: | --- |
| DNS 查询 | 1～4 | `h1` 同时查询域名的 IPv4（A）和 IPv6（AAAA）地址，A 记录返回 `124.16.77.5` |
...
DNS 查询从 `10.0.0.1` 发往 DNS 转发节点 `10.255.255.254`，约 30 ms 后收到了 IPv4 地址响应。随后 `10.0.0.1` 使用临时端口 `53446` 连接服务器 `124.16.77.5` 的 HTTPS 端口 `443`。TCP 三次握手完成后进行 TLS 握手，之后 `wget` 的 HTTP 请求和服务器返回的页面内容都以加密数据形式传输。

ARP 数据包出现在抓包末尾，说明开始下载时所需的 MAC 地址很可能已经存在于缓存中，因而不需要在 TCP 连接之前重新查询。后续 ARP 请求仍然展示了 IP 地址到 MAC 地址的解析过程。

#### 3.4.1 TCP 三次握手

建立 TCP 连接的过程称“三次握手”。抓包开头出现的几个方向交替的数据包，含义如下。设客户端的初始序号为 $x$，服务器的初始序号为 $y$：

| 数据包编号 | 方向 | 标志位 | 序号与确认号 | 含义 |
| ---: | --- | --- | --- | --- |
| 5 | 客户端 → 服务器 | SYN | `Seq=x` | 客户端请求建立连接，并给出自己的初始序号 |
| 6 | 服务器 → 客户端 | SYN, ACK | `Seq=y, Ack=x+1` | 服务器同意连接，确认客户端的请求，同时给出自己的初始序号 |
| 7 | 客户端 → 服务器 | ACK | `Seq=x+1, Ack=y+1` | 客户端确认服务器的请求，连接建立完成 |
...
这也解释了为什么 Wireshark 没有直接显示 `GET` 请求和网页正文：它们位于 TLS 加密数据内部。Wireshark 仍然可以显示通信方向、数据长度和 TLS 记录类型，但在没有会话密钥的情况下不能还原其中的 HTTP 明文。
...
结束连接的过程通常称为“四次挥手”。本次抓包只显示了 3 个包，是因为服务器把中间的“确认客户端 FIN”和“发送自己的 FIN”合并在同一个 `FIN, ACK` 包中：

| 数据包编号 | 方向 | 标志位 | 含义 |
| ---: | --- | --- | --- |
| 67 | 客户端 → 服务器 | FIN, ACK | 客户端表示网页已经接收完成，不再发送新的应用数据 |
| 68 | 服务器 → 客户端 | FIN, ACK | 服务器确认客户端的 FIN，同时表示自己也不再发送新的应用数据 |
| 69 | 客户端 → 服务器 | ACK | 客户端确认服务器的 FIN，连接关闭 |
...
1. `wget` 根据 URL 得到域名 `www.ucas.ac.cn`。
2. DNS 将域名解析为服务器 IP 地址 `124.16.77.5`。
3. 主机利用 ARP 获得下一跳设备的 MAC 地址，在以太网中发送数据帧。
4. 客户端与服务器的 443 端口进行 TCP 三次握手。
5. 双方进行 TLS 握手，建立加密连接并验证服务器身份。
6. 客户端发送加密的 HTTP 请求，服务器返回加密的网页内容。
7. 下载完成后，双方关闭 TCP 连接。
...
- **Ping**：向实际连接的服务器 IP 发送 3 次 ICMP Echo，取最小往返时间。
- **DNS**：完成域名解析所需的时间。
- **TCP transfer**：从收到 HTTP 响应的第一个字节到接收完最后一个字节的时间。
- **Total time**：从开始请求到完整接收响应的总时间。

脚本还使用 GeoIP 数据库估算服务器的地理位置，并计算测量点到服务器的大圆距离。将信号按真空光速往返传播所需的时间记为 `c-latency`：

$$
c\text{-latency}=\frac{2\times \text{distance}}{c}
$$

其中 $c=299792.458\ \text{km/s}$。`c-latency` 是只考虑地理距离的理想下界，实际网络时延通常会大于它。
...
访问测量主要使用 `pycurl` 提供的计时值。核心计算可简化为：
...
这里将“第一个响应字节到达以后，直到响应接收完成”的时间作为 TCP data transfer。Ping 部分使用系统的 `ping` 命令向网站实际连接的 IP 发送 3 次请求，并读取最小 RTT。这样测得的 Ping 与 HTTP 请求访问的是同一个服务器地址，便于比较。
...
实际上进行了分阶段实验：本地先测试 1000 个网站，严格清洗数据时保留的数据在 100 多条，于是尝试 5000 条，大约半小时才完成，保留数据 700 多条。一方面绘制出的图形相对阶梯状明显，而且出现很明显的波浪式上升，猜测是数据量不够。最终是开了一个托管在阿里云的远程机跑了三个多小时跑出接近 3000 条有效数据，图形就相对平滑了。

#### 4.3.2 数据清洗逻辑

原始数据包含 20,000 个网站。由于部分网站无法访问、拒绝 Ping、返回错误状态码，或者缺少可靠的 GeoIP 信息，不能直接把所有记录用于比较。清洗脚本按以下顺序检查每条数据：

1. 五个数值字段 `c-latency`、Ping、DNS、TCP transfer 和 Total time 必须完整。
2. 目标地址必须是有效的公网 IP。
3. HTTP 状态码必须在 200～399 范围内，只保留正常响应或重定向。
4. 各项数值必须大于 0，且 Total time 不能小于 DNS 与 TCP transfer 之和，否则各阶段时间关系不合理。
5. Ping 不应小于按真空光速计算的 c-latency；如果出现这种情况，通常说明 GeoIP 位置不准确。
6. GeoIP 给出的定位精度半径不能超过 200 km，并且测量点到服务器的距离应大于该精度半径。

本次清洗结果如下。每条被剔除的记录只统计其首先发现的主要原因：
...
| **合计剔除** | **17,091** | — |
| **最终保留** | **2,909** | 五项指标完整、状态正常且地理信息较可靠 |

### 4.4 测量结果

下图横轴表示“实测时延 ÷ c-latency”，即实际网络相对于理想传播下界慢了多少倍；纵轴是累计分布 CDF。例如，CDF 等于 0.5 处的横坐标就是该指标的中位数。

![网站时延相对 c-latency 的累计分布](ping_latency_graph/latency_inflation_cdf_20000_clean_geo200_distance_ecdf.png)

清洗后 2,909 个样本的主要结果如下：

| 指标 | 实测时间中位数 | 相对 c-latency 的中位倍数 | 简单理解 |
| --- | ---: | ---: | --- |
| Ping RTT | 185.729 ms | 4.0× | 最接近单纯的网络往返传播时间 |
| DNS lookup | 435.765 ms | 11.1× | 还受到 DNS 服务器和缓存状态影响 |
| TCP data transfer | 159.109 ms | 4.5× | 受到响应大小、带宽和拥塞等影响 |
| Total HTTP time | 2401.232 ms | 58.6× | 包含解析、建连、TLS、服务器处理和数据下载等完整过程 |

为了进一步观察分布而不仅仅比较中位数，可以读取曲线在不同累计比例处的横坐标：

| 指标 | 25% 样本不超过 | 50% 样本不超过 | 75% 样本不超过 | 90% 样本不超过 |
| --- | ---: | ---: | ---: | ---: |
| Ping RTT | 3.0× | 4.0× | 5.2× | 8.0× |
| DNS lookup | 4.8× | 11.1× | 26.1× | 58.9× |
| TCP data transfer | 0.03× | 4.5× | 13.1× | 33.1× |
| Total HTTP time | 32.3× | 58.6× | 125.8× | 219.5× |

#### 4.4.1 图中趋势和分布规律

横轴采用对数坐标，因此曲线越靠左，表示该指标越接近理想传播下界；曲线上升越陡，表示多数样本越集中；向右延伸较长，则表示不同网站之间的差异较大。

- **Ping RTT（蓝色）最靠左且上升较陡。** 它的中位数约为 4.0×，90% 的样本不超过 8.0×。这说明 Ping 的分布相对集中，主要反映网络路径上的传播、转发和排队时延。
- **DNS lookup（黑色）比 Ping 更靠右，也更分散。** 中位数为 11.1×，到 90% 分位时达到 58.9×。不同域名的缓存状态、DNS 服务器位置以及是否需要继续向上游查询都会造成较大的差别。
- **TCP data transfer（灰色）在左端有明显的一段快速上升，之后又形成较长的右尾。** 约 34.7% 的样本小于 1×，绘图时被集中显示在 1× 的左边界。出现这种现象并不表示数据传播超过光速，因为该指标只计算“第一个响应字节到最后一个响应字节”的间隔；对于响应很小的网站，这段时间可以远小于一次完整 RTT。另一方面，大网页、低带宽或拥塞会把部分样本推向右侧，因此它的离散程度较大。
- **Total HTTP time（橙色）整体最靠右。** 其中位数达到 58.6×，90% 的样本不超过约 219.5×，并且存在明显长尾。它包含一次网页访问的多个阶段，任何一个阶段变慢都会累积到总时间中。
...
1. 光在光纤中的传播速度低于真空光速，而且实际光纤线路通常不是两地之间的直线。
2. 数据包需要经过多个路由器和交换设备，每一跳都会产生处理和排队时间。
3. DNS 查询可能需要访问缓存或上游 DNS 服务器，因此它不仅包含简单的链路传播时间。
4. TCP 数据传输会受到网络带宽、网页大小、拥塞控制和丢包重传等因素影响。
5. Total time 还包含 DNS、TCP 建连、TLS 握手、服务器处理、重定向和页面下载，所以其倍数最大。
